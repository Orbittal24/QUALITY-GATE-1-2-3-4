...
const TableThreemodule: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPackNameAndQRCode = async () => {
      try {
        const response = await axios.get(`/api/getPackNamemodule?customerQRCode=${customerQRCode}`);
        ...
        setAlertMessage('Error fetching battery pack name and final QR code');
      }
    };
  ...
  useEffect(() => {
    const fetchTableNames = async () => {
      try {
        const response = await axios.get(`/api/adminchecklistmodule?packName=${batteryPackName}`);
  ...
  const handleSelectTableName = async (tableName: string) => {
    try {
      const response = await axios.get(`/api/adminchecklistmodule?tableName=${tableName}`);
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post('/api/usechecklistmodule', {
...
export default TableThreemodule;
